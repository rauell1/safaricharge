...
export default function ModularDashboardDemo() {
  ...
  const handleExportReport = useCallback(async () => {
    try {
        ...
        return Array.from(groups.entries())
          .map(([period, items]) => {
            ...
            return {
              ...
              totalHomeLoadKWh: items.reduce(
                (s, d) => s + (d.homeLoadKWh ?? (d.homeLoadKW || 0) * (24 / 420)),
                0
              ),
              totalEV1LoadKWh: items.reduce(
                (s, d) => s + (d.ev1LoadKWh ?? (d.ev1LoadKW || 0) * (24 / 420)),
                0
              ),
              totalEV2LoadKWh: items.reduce(
                (s, d) => s + (d.ev2LoadKWh ?? (d.ev2LoadKW || 0) * (24 / 420)),
                0
              ),
              totalGridImportKWh: items.reduce((s, d) => s + (d.gridImportKWh || 0), 0),
              totalGridExportKWh: items.reduce((s, d) => s + (d.gridExportKWh || 0), 0),
              avgBatteryLevelPct:
                c > 0 ? items.reduce((s, d) => s + (d.batteryLevelPct || 0), 0) / c : 0,
              avgEV1SocPct:
                c > 0 ? items.reduce((s, d) => s + (d.ev1SocPct || 0), 0) / c : 0,
              avgEV2SocPct:
                c > 0 ? items.reduce((s, d) => s + (d.ev2SocPct || 0), 0) / c : 0,
      ...
      const hourlyData = aggregate(
        (d) => `${d.date} ${String(d.hour).padStart(2, '0')}:00`
      );
      const dailyData = aggregate((d) => d.date);
      const weeklyData = aggregate(
        (d) => `${d.year}-W${String(d.week).padStart(2, '0')}`
      );
      const monthlyData = aggregate(
      ...
      const totalHomeLoad = minuteData.reduce(
        (s, d) => s + (d.homeLoadKWh ?? (d.homeLoadKW || 0) * (24 / 420)),
        0
      );
      const totalEV1Load = minuteData.reduce(
        (s, d) => s + (d.ev1LoadKWh ?? (d.ev1LoadKW || 0) * (24 / 420)),
        0
      );
      const totalEV2Load = minuteData.reduce(
        (s, d) => s + (d.ev2LoadKWh ?? (d.ev2LoadKW || 0) * (24 / 420)),
        0
      );
      ...
      parts.push(
        `Date Range,${minuteData[0]?.date || 'N/A'},to,${minuteData[minuteData.length - 1]?.date || 'N/A'}`
      );
      parts.push('');
      ...
      parts.push(
        `Net Energy,${(totalSolar - totalGridImport + totalGridExport).toFixed(2)},kWh`
      );
      parts.push(
        `Self-Sufficiency Rate,${
          totalLoad > 0 ? ((totalSolar / totalLoad) * 100).toFixed(1) : 0
        },%`
      );
      ...
      for (const d of minuteData) {
        parts.push(
          `${d.timestamp},${d.date},${d.year},${d.month},${d.week},${d.day},${d.hour},${d.minute},${(d.solarKW || 0).toFixed(2)},${(d.homeLoadKW || 0).toFixed(2)},${(d.ev1LoadKW || 0).toFixed(2)},${(d.ev2LoadKW || 0).toFixed(2)},${(d.batteryPowerKW || 0).toFixed(2)},${(d.batteryLevelPct || 0).toFixed(1)},${(d.gridImportKW || 0).toFixed(2)},${(d.gridExportKW || 0).toFixed(2)},${(d.ev1SocPct || 0).toFixed(1)},${(d.ev2SocPct || 0).toFixed(1)},${(d.tariffRate || 0).toFixed(2)},${d.isPeakTime ? 'Yes' : 'No'},${(d.savingsKES || 0).toFixed(2)},${(d.solarEnergyKWh || 0).toFixed(4)},${(d.gridImportKWh || 0).toFixed(4)},${(d.gridExportKWh || 0).toFixed(4)}`
        );
      }
      parts.push('');
      ...
      const writeSection = (title: string, data: AggRow[], periodLabel: string) => {
        parts.push(title);
        parts.push(
          `${periodLabel},Total Solar (kWh),Total Home Load (kWh),Total EV1 Load (kWh),Total EV2 Load (kWh),Grid Import (kWh),Grid Export (kWh),Avg Battery (%),Avg EV1 SoC (%),Avg EV2 SoC (%),Savings (KES),Peak Count,Off-Peak Count`
        );
        for (const d of data) {
          parts.push(
            `${d.period},${d.totalSolarKWh.toFixed(2)},${d.totalHomeLoadKWh.toFixed(2)},${d.totalEV1LoadKWh.toFixed(2)},${d.totalEV2LoadKWh.toFixed(2)},${d.totalGridImportKWh.toFixed(2)},${d.totalGridExportKWh.toFixed(2)},${d.avgBatteryLevelPct.toFixed(1)},${d.avgEV1SocPct.toFixed(1)},${d.avgEV2SocPct.toFixed(1)},${d.totalSavingsKES.toFixed(2)},${d.peakHoursCount},${d.offPeakHoursCount}`
          );
        }
        parts.push('');
      };
  ...
  const handleFormalReport = useCallback(async () => {
    ...
    try {
      ...
      if (!reportWindow) {
        alert(
          'Unable to open the report. Please allow pop-ups for this site and try again.'
        );
        return;
      }
      reportWindow.document.write(
        '<html><body style="font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;background:#0f172a;color:#94a3b8;"><p>Generating report\u2026</p></body></html>'
      );
      ...
      const totalEV1 = minuteData.reduce(
        (s, d) => s + (d.ev1LoadKWh ?? (d.ev1LoadKW ?? 0) * (24 / 420)),
        0
      );
      const totalEV2 = minuteData.reduce(
        (s, d) => s + (d.ev2LoadKWh ?? (d.ev2LoadKW ?? 0) * (24 / 420)),
        0
      );

      let peakGridImport = 0;
      let peakInstantSolar = 0;
      let peakEVLoad = 0;
      let batterySum = 0;
      const peakBreakdown = {
        records: 0,
      ...
      for (const d of minuteData) {
        ...
        const evEnergy =
          (d.ev1LoadKWh ?? (d.ev1LoadKW ?? 0) * (24 / 420)) +
          (d.ev2LoadKWh ?? (d.ev2LoadKW ?? 0) * (24 / 420));
      ...
      const dailyAgg = Array.from(dailyMap.values())
        .map((r) => ({
          ...r,
          avgBattery: r.batteryCount > 0 ? r.avgBattery / r.batteryCount : 0,
        }))
        .sort((a, b) => a.date.localeCompare(b.date));
      const uniqueDays = dailyAgg.length;
      const dailyAggCharts = dailyAgg.slice(-30);

      const { createLoadProfileFromSimulation, generateRecommendation } = await import(
      ...
      const startDate =
        minuteData[0]?.date ?? new Date().toISOString().slice(0, 10);
      ...
      if (!response.ok) {
        ...
        if (rawError) {
          try {
            const errorData = JSON.parse(rawError) as {
              error?: string;
              message?: string;
              details?: string;
            };
  ...
  const flowDirection = useMemo(
    () => ({
      solarToHome: flows.some(
        (f) => f.from === 'solar' && f.to === 'home' && f.active
      ),
      solarToBattery: flows.some(
        (f) => f.from === 'solar' && f.to === 'battery' && f.active
      ),
      solarToGrid: flows.some(
        (f) => f.from === 'solar' && f.to === 'grid' && f.active
      ),
      batteryToHome: flows.some(
        (f) => f.from === 'battery' && f.to === 'home' && f.active
      ),
      gridToHome: flows.some(
        (f) => f.from === 'grid' && f.to === 'home' && f.active
      ),
    }),
    [flows]
  );
  ...
  const envImpact = useMemo(
    () => [
      {
        icon: Leaf,
        value: `${(accumulators.carbonOffset / 1000).toFixed(2)} tons`,
      ...
      {
        icon: Trees,
        value: Math.round(accumulators.carbonOffset / 14).toString(),
      ...
      {
        icon: Car,
        value: `${Math.round(accumulators.carbonOffset * 1.6)} km`,
  ...
  const sparklineData = useMemo(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (dayData.length > 0) {
        dailyData.gen.push(
          dayData.reduce((sum, d) => sum + d.solarEnergyKWh, 0)
        );
        dailyData.cons.push(
          dayData.reduce(
        ...
        dailyData.savings.push(
          dayData.reduce((sum, d) => sum + d.savingsKES, 0)
        );
        dailyData.power.push(
          dayData.reduce((sum, d) => sum + d.solarKW, 0) / dayData.length
        );
      }
    }
  ...
  const trendsData = useMemo(() => {
    ...
    const usefulEnergy =
      Math.min(homePower, solarPower) +
      (batteryPower > 0 ? Math.min(batteryPower, solarPower - homePower) : 0);
    const systemEfficiency =
      solarPower > 0 ? (usefulEnergy / solarPower) * 100 : 0;
    const savingsChange =
      yesterdaySavings > 0
    ...
    const batteryOptimized = isPeakHour
      ? batteryLevel > 70
      : batteryLevel > 50;
    ...
    <DashboardLayout>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6 lg:px-8">
        <div className="max-w-7xl mx-auto space-y-6 lg:space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-[var(--text-tertiary)]">
                Monitor your solar energy system performance
              </p>
            </div>
            <TimeRangeSwitcher
                    ...
                    <div className="relative flex h-40 w-40 items-center justify-center">
                      ...
                      >
                        <circle
                          ...
                          stroke="var(--solar)"
                          strokeWidth="14"
                          strokeDasharray={`${ringSegments.solar} ${ringSegments.circumference}`}
                          strokeLinecap="round"
                          opacity="0.9"
                        />
                        <circle
                        ...
                        <circle
                          ...
                          strokeDashoffset={`${
                            -(ringSegments.solar + ringSegments.consumption)
                          }`}
                          strokeLinecap="round"
                          opacity="0.9"
                        />
                      ...
                      {[
                        {
                          ...
                          pct: Math.round(energySplit.solarPct * 100),
                          color: 'var(--solar)',
                        },
                        {
                          label: 'Consumption',
                          pct: Math.round(energySplit.consumptionPct * 100),
                          color: 'var(--consumption)',
                        },
                        {
                          label: 'Grid Export',
                          pct: Math.round(energySplit.exportPct * 100),
                          color: 'var(--grid)',
                        },
                      ].map((item) => (
                        <div
          ...
          {/* ROW 5 — Panel Status + Weather & Battery */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              <PanelStatusTable />
            </div>
            ...
            <CardContent>
              <div className="flex items-end justify-between gap-2 h-44 px-2">
                {[
                  ...
                  const gen = [
                    65, 70, 78, 85, 90, 95, 88, 92, 80, 75, 68, 62,
                  ][i];
                  const cons = [
                    55, 58, 60, 62, 65, 68, 70, 69, 65, 60, 57, 54,
                  ][i];
                  return (
                    <div
                      key={month}
              ...
              <div className="mt-4 flex items-center justify-center gap-6">
                <div className="flex items-center gap-1.5">
                  <div
                    ...
                    style={{ backgroundColor: 'var(--solar)' }}
                  />
                  <span className="text-xs text-[var(--text-secondary)]">
                    Generation (kWh)
                  </span>
                ...
                <div className="flex items-center gap-1.5">
                  <div
                    className="h-2.5 w-2.5 rounded-sm"
                    style={{ backgroundColor: 'var(--consumption)' }}
                  />
                  <span className="text-xs text-[var(--text-secondary)]">
                    Consumption (kWh)
                  </span>
          ...
          <EngineeringKPICard
            ...
            pvCapacityLabel={`${
              solarNode.capacityKW?.toFixed(1) ?? '\u2014'
            } kWp installed`}
          />
        </div>
      </main>
