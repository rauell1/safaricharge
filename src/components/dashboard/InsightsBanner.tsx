  ...
  const insights = useMemo(() => {
    ...
    if (systemEfficiency >= 85) {
      items.push({
        ...
        text: `System operating at ${systemEfficiency.toFixed(0)}% efficiency today`,
        tone: 'positive',
      });
    } else if (systemEfficiency >= 70) {
      items.push({
        icon: <TrendingUp className="h-5 w-5" />,
        text: `System efficiency at ${systemEfficiency.toFixed(0)}% - room for optimization`,
        tone: 'neutral',
      });
    } else {
      items.push({
        icon: <AlertCircle className="h-5 w-5" />,
        text: `System efficiency at ${systemEfficiency.toFixed(0)}% - needs attention`,
        tone: 'warning',
      });
    }

    // Savings insight
    const savingsChangeText = savingsChange >= 0
      ? `↑ ${savingsChange.toFixed(0)}% vs yesterday`
      : `↓ ${Math.abs(savingsChange).toFixed(0)}% vs yesterday`;

    items.push({
      icon: <DollarSign className="h-5 w-5" />,
      text: `Estimated savings: KES ${todaySavings.toFixed(0)} ${savingsChangeText}`,
      tone: savingsChange >= 0 ? 'positive' : 'neutral',
    });
    ...
    if (Math.abs(forecastChange) >= 5) {
      ...
      items.push({
        ...
        text: `Tomorrow forecast: ${changeText} solar generation expected`,
        tone: forecastChange > 0 ? 'info' : 'neutral',
      });
    }
    ...
    if (batteryOptimized) {
      items.push({
        ...
        text: 'Battery optimized for evening peak usage',
        tone: 'positive',
      });
    }
    ...
    if (alertCount > 0) {
      items.push({
        icon: <AlertCircle className="h-5 w-5" />,
        text: `${alertCount} system alert${alertCount > 1 ? 's' : ''} require${alertCount === 1 ? 's' : ''} attention`,
        tone: 'warning',
      });
    }
