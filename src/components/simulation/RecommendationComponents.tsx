        ...
        <div className="absolute top-full right-0 mt-2 w-[min(20rem,calc(100vw-1.5rem))] bg-white rounded-lg shadow-xl border border-slate-200 z-50 overflow-hidden">
          ...
          <div className="max-h-96 overflow-y-auto">
            ...
            <div className="p-3 border-t border-slate-200 bg-white">
              <button
                onClick={() => { if (onInvalidateCache) onInvalidateCache(); if (!cacheRef) localCacheRef.current = {}; setDataSourceLabel(''); }}
...
export const RecommendationPanel: React.FC<RecommendationPanelProps> = ({
  ...
  const [expanded, setExpanded] = useState<{[key: string]: boolean}>({
    solar: true, battery: true, inverter: true, financial: true
  });
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <div className="p-4 bg-gradient-to-r from-sky-600 to-sky-700 text-white flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-1 rounded-full bg-white/10 text-[10px] font-semibold border border-white/20">Last updated: {updatedLabel}</span>
            ...
            <button
              onClick={() => { if (simulationData.length === 0) { setError('Run the simulation first to generate recommendations.'); return; } setError(null); onGenerate(); }}
        ...
        <div className="flex-1 overflow-auto p-6">
          {isGenerating ? (
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex items-center gap-2 text-xs">
                      <span className={`px-2 py-0.5 rounded-full font-bold ${ recommendation.confidence === 'high' ? 'bg-green-200 text-green-800' : recommendation.confidence === 'medium' ? 'bg-yellow-200 text-yellow-800' : 'bg-orange-200 text-orange-800' }`}>
                ...
                <button onClick={() => toggleSection('solar')} className="w-full p-4 bg-gradient-to-r from-yellow-50 to-orange-50 flex items-center justify-between hover:from-yellow-100 hover:to-orange-100 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-sm text-slate-600">{recommendation.solarPanels.totalCapacityKw} kW • {recommendation.solarPanels.numberOfPanels} panels</p>
                    </div>
                  </div>
                ...
                {expanded.solar && (
                  <div className="p-4 bg-white space-y-3">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div><p className="text-xs text-slate-500 mb-1">Number of Panels</p><p className="text-lg font-bold text-slate-900">{recommendation.solarPanels.numberOfPanels} × {recommendation.solarPanels.panelWattage}W</p></div>
              ...
              {/* Battery */}
              <div className="border border-slate-200 rounded-lg overflow-hidden">
                <button onClick={() => toggleSection('battery')} className="w-full p-4 bg-gradient-to-r from-purple-50 to-pink-50 flex items-center justify-between hover:from-purple-100 hover:to-pink-100 transition-colors">
                  <div className="flex items-center gap-3">
                    <Battery size={24} className="text-purple-500" />
                    <div className="text-left">
                      <h3 className="font-bold text-slate-900">Battery Storage</h3>
                      <p className="text-sm text-slate-600">{recommendation.battery.capacityKwh} kWh • {recommendation.battery.typeRecommended}</p>
                    </div>
                  </div>
                ...
                {expanded.battery && (
                  <div className="p-4 bg-white space-y-3">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div><p className="text-xs text-slate-500 mb-1">Capacity</p><p className="text-lg font-bold text-slate-900">{recommendation.battery.capacityKwh} kWh</p></div>
                      <div><p className="text-xs text-slate-500 mb-1">Battery Type</p><p className="text-sm font-bold text-slate-900">{recommendation.battery.typeRecommended}</p></div>
                      <div><p className="text-xs text-slate-500 mb-1">Configuration</p><p className="text-sm font-bold text-slate-900">{recommendation.battery.numberOfBatteries} × {recommendation.battery.batteryCapacityAhPer}Ah @ {recommendation.battery.voltageSystem}V</p></div>
                      <div><p className="text-xs text-slate-500 mb-1">Estimated Cost</p><p className="text-lg font-bold text-slate-900">KES {recommendation.battery.estimatedCostKES.toLocaleString()}</p></div>
                    </div>
                  </div>
                )}
              </div>
              ...
              {/* Inverter */}
              <div className="border border-slate-200 rounded-lg overflow-hidden">
                <button onClick={() => toggleSection('inverter')} className="w-full p-4 bg-gradient-to-r from-sky-50 to-blue-50 flex items-center justify-between hover:from-sky-100 hover:to-blue-100 transition-colors">
                  <div className="flex items-center gap-3">
                    <Zap size={24} className="text-sky-500" />
                    <div className="text-left">
                      <h3 className="font-bold text-slate-900">Inverter</h3>
                      <p className="text-sm text-slate-600">{recommendation.inverter.ratedCapacityKw} kW • {recommendation.inverter.typeRecommended}</p>
                    </div>
                  </div>
                ...
                {expanded.inverter && (
                  <div className="p-4 bg-white space-y-3">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div><p className="text-xs text-slate-500 mb-1">Rated Capacity</p><p className="text-lg font-bold text-slate-900">{recommendation.inverter.ratedCapacityKw} kW</p></div>
                      <div><p className="text-xs text-slate-500 mb-1">Type</p><p className="text-sm font-bold text-slate-900">{recommendation.inverter.typeRecommended}</p></div>
                      <div className="col-span-2"><p className="text-xs text-slate-500 mb-1">Estimated Cost</p><p className="text-lg font-bold text-slate-900">KES {recommendation.inverter.estimatedCostKES.toLocaleString()}</p></div>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="border-2 border-green-200 rounded-lg overflow-hidden bg-gradient-to-br from-green-50 to-emerald-50">
                <button onClick={() => toggleSection('financial')} className="w-full p-4 flex items-center justify-between hover:bg-green-100 transition-colors">
                  <div className="flex items-center gap-3">
                    <TrendingUp size={24} className="text-green-600" />
                    <div className="text-left">
                      <h3 className="font-bold text-slate-900">Financial Analysis</h3>
                      <p className="text-sm text-slate-600">{recommendation.financial.paybackPeriodYears} year payback • {recommendation.financial.roi25YearsPct}% ROI</p>
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-white space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="sm:col-span-2 p-3 bg-slate-50 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-slate-900">KES {recommendation.financial.totalInvestmentKES.toLocaleString()}</p>
                        <p className="text-xs text-slate-500 mt-1">Equipment: KES {recommendation.financial.totalSystemCostKES.toLocaleString()} + Installation: KES {recommendation.financial.installationCostKES.toLocaleString()}</p>
                      </div>
                      <div><p className="text-xs text-slate-500 mb-1">Monthly Savings</p><p className="text-lg font-bold text-green-600">KES {recommendation.financial.monthlyGridSavingsKES.toLocaleString()}</p></div>
                      <div><p className="text-xs text-slate-500 mb-1">Annual Savings</p><p className="text-lg font-bold text-green-600">KES {recommendation.financial.annualGridSavingsKES.toLocaleString()}</p></div>
                      <div><p className="text-xs text-slate-500 mb-1">Payback Period</p><p className="text-lg font-bold text-sky-600">{recommendation.financial.paybackPeriodYears} years</p></div>
                      <div><p className="text-xs text-slate-500 mb-1">25-Year ROI</p><p className="text-lg font-bold text-sky-600">{recommendation.financial.roi25YearsPct}%</p></div>
                      <div><p className="text-xs text-slate-500 mb-1">NPV (12% discount)</p><p className="text-lg font-bold text-emerald-700">KES {recommendation.financial.npvKES.toLocaleString()}</p></div>
                      <div><p className="text-xs text-slate-500 mb-1">IRR</p><p className="text-lg font-bold text-emerald-700">{recommendation.financial.irrPct}%</p></div>
                      <div className="sm:col-span-2"><p className="text-xs text-slate-500 mb-1">LCOE</p><p className="text-lg font-bold text-emerald-700">KES {recommendation.financial.lcoeKESPerKwh.toLocaleString()} / kWh</p></div>
                      <div className="sm:col-span-2 p-3 bg-green-50 rounded-lg">
                        <p className="text-xs text-slate-500 mb-1">Net Savings (25 Years)</p>
                        <p className="text-2xl font-bold text-green-600">KES {recommendation.financial.netSavings25YearsKES.toLocaleString()}</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="border border-slate-200 rounded-lg p-4 bg-gradient-to-br from-green-50 to-teal-50">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><p className="text-xs text-slate-500 mb-1">Grid Dependency Reduction</p><p className="text-lg font-bold text-slate-900">{recommendation.performance.gridDependencyReductionPct}%</p></div>
                  <div><p className="text-xs text-slate-500 mb-1">Daily Solar Generation</p><p className="text-lg font-bold text-slate-900">{recommendation.performance.dailySolarGenerationKwh} kWh</p></div>
                  <div><p className="text-xs text-slate-500 mb-1">Annual CO₂ Savings</p><p className="text-lg font-bold text-green-600">{recommendation.performance.annualCO2SavingsKg.toLocaleString()} kg</p></div>
                  <div><p className="text-xs text-slate-500 mb-1">Equivalent Trees Planted</p><p className="text-lg font-bold text-green-600">{recommendation.performance.equivalentTreesPlanted} trees</p></div>
                </div>
              </div>
            ...
            <div className="text-center py-12 text-slate-500 space-y-3">
              ...
              <p className="text-sm text-slate-700 text-justify [text-align-last:left]">Select a location and click “Generate” to create a recommendation using {activeSourceLabel} data for {currentLocation.name}.</p>
    ...
    <>
      {minuteData.length === 0 && (
        <div className="flex items-start gap-3 rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-800 dark:border-sky-800 dark:bg-sky-950 dark:text-sky-200">
          ...
          <span><strong>Tip:</strong> Run the simulation first for more accurate, data-driven recommendations. Without simulation data the engine uses default load assumptions.</span>
        </div>
      )}
