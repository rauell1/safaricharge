    ...
    <SheetPortal>
      ...
      >
        ...
        <SheetPrimitive.Close
          className={cn(
            "absolute top-4 right-4 rounded-lg p-1.5 opacity-60",
            "text-[var(--text-secondary)]",
            "hover:opacity-100 hover:bg-[var(--bg-card-muted)] hover:text-[var(--text-primary)]",
            "focus:ring-2 focus:ring-[var(--battery)] focus:ring-offset-2 focus:outline-none",
            "disabled:pointer-events-none transition-all duration-150",
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("mt-auto flex flex-col gap-2 sm:flex-row sm:justify-end p-6", className)}
      {...props}
    />
  )
}
